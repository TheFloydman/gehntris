...
class PieceInPlay {
    ...
    draw(x, y, board) {
        ...
        for (let i = 0; i < this.squares.length; i++) {
            ...
            if (this.waiting)
                svgElement = svgUpcoming;
            if (this.stored)
                svgElement = svgStorage;
...
function setBoardProperties() {
    ...
    svgUpcoming = document.getElementById("svg-upcoming");
    svgUpcoming.setAttribute("viewBox", `0 0 ${upcomingSize.x * squareSize} ${upcomingSize.y * squareSize}`);
    svgStorage = document.getElementById("svg-storage");
...
function addPieceToBoard(fromStorage = false) {
    if (fromStorage) {
...
//# sourceMappingURL=script-0.3.0.js.map